...
class Particle:
    ...
    def observation_model(self, lx, ly):
        ...
        a = np.array([[lx-x],[ly-y]]).reshape(2,1)
        q = (a.T).dot(a)
        z_r = np.sqrt(q)
    ...
    def update_landmark(self, z, lid):
        ...
        if lid not in self.landmarks:
            ...
            # [Hint 3] The mean of landmark "mu" is a numpy array with shape (2,1)2*1-matrix.
            ...
            sig = Hinv @ self.Qt @ Hinv.T #(Hinv.dot(self.Qt)).dot(Hinv.T)
            ...
            e = (np.array(z).reshape(2,1) - np.array(z_hat).reshape(2,1))
...
def multi_normal(x, mean, cov):
    ...
    a = (math.exp(-0.5 * (err).T @ np.linalg.inv(cov) @ (err))) / (2 * np.pi * np.sqrt(np.linalg.det(cov)))
